...
Option Explicit
Private Const pxeMODULENAME = "frmCategory"
...
Private Sub Form_Load()
   ...
   oDriver.BrowseFTitle(0) = "Category ID"
   oDriver.BrowseFTitle(1) = "Category Name"
   oDriver.BrowseFTitle(2) = "Category Level"
...
Private Sub oDriver_DisableOtherControl()
   ...
   optPrimary(0).Enabled = False
   optPrimary(1).Enabled = False
   optPrimary(2).Enabled = False
End Sub

Private Sub oDriver_EnableOtherControl()
   oDriver.DisableTextbox 0
   chkField.Enabled = True
   optPrimary(0).Enabled = True
   optPrimary(1).Enabled = True
   optPrimary(2).Enabled = True
End Sub
...
Private Sub oDriver_InitValue()
   ...
   If oDriver.SetValue(0, GetNextCode("Category", "sCategrID", False, oApp.Connection, True, oApp.BranchCode)) = False Then Exit Sub
   oDriver.FieldReference(0) = True
   oDriver.FieldValue(2) = 1
   oDriver.FieldValue(3) = 0
   oDriver.FieldValue(4) = 1
   
   chkField.Value = oDriver.FieldValue(2)
   
   Select Case oDriver.FieldValue(3)
   Case 0 'Primary
      optPrimary(0).Value = True
   Case 1 'Sub cat 1
      optPrimary(1).Value = True
   Case 2 'sub cat 2
      optPrimary(2).Value = True
   End Select
...
Private Sub oDriver_WillSave(Cancel As Boolean)
   If oDriver.FieldValue(1) = "" Then
      MsgBox "Invalid Category Name detected!!!", vbCritical, "Warning"
      txtField(1).SetFocus
      Cancel = True
   End If
   
   oDriver.FieldValue(2) = chkField.Value
   
   Select Case optPrimary(pnCategory)
   Case 0 'Primary
      oDriver.FieldValue(3) = pnCategory + 1
   Case 1 'Sub cat 1
      oDriver.FieldValue(3) = pnCategory + 1
   Case 2 'sub cat 2
      oDriver.FieldValue(3) = pnCategory + 1
...
Private Sub optPrimary_Click(Index As Integer)
   pnCategory = Index
End Sub
